...
export class Text extends MyObject3D {
  ...
  constructor(opt: {id: number, color: Color, useMask: boolean, scale: number}) {
    ...
    const tex = TexLoader.instance.get(Conf.instance.PATH_IMG + 'tex-photo.png');
  ...
  protected _update():void {
    ...
    this.position.z = Util.instance.map(this._noise.x, 1, -1, 0, Conf.instance.TEXT_NUM - 1) * s * 0.1;
